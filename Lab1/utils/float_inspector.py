...
def print_binary_float_html(fl, word, exp, mantissa):
        ...
        html = "<h3> {fl:f} </h3>".format(fl=fl)
        
        html += '<table width="100%"> '
        html += '<tr> <td width="10%"></td> <td width="5%"> <b>  </b> </td> <td> <b>  </b> </td> <td> <b>  </b> </td> </tr>'
        html += '<tr> <td width="5%"> Raw  </td> <td colspan="3" > {word} </td> '.format(word=str(bar_word))
        ...
        html += "<tr> <td> Power </td> <td> {sign} </td> <td> {mantissa} </td> <td>  {exponent} </td> </tr>".format(sign=html_sign, mantissa=html_mantissa,
        exponent=html_exponent)

        if not infinite:
            html += "<tr> <td> = </td> <td> {0}  </td> <td> {1} </td> <td> {2} </td> </tr>".format(sign, 1.0+m/(2.0**mantissa), 2**e)
        html += '<tr> <td> Float </td> <td rowspan="3"> {fl:.20f} </td>'.format(fl=fl)
        html += "</table>"
